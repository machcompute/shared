...
export class GPU {
  ...
  upload(buffer, data, offset = 0) {
    ...
    if ((offset & 3) !== 0) throw new Error(`GPU upload offset ${offset} is not 4-byte aligned`);
    if (byteLength != null && (byteLength & 3) !== 0) {
      const padded = new Uint8Array((byteLength + 3) & ~3);
      padded.set(new Uint8Array(data.buffer ?? data, data.byteOffset ?? 0, byteLength));
      this.device.queue.writeBuffer(buffer, offset, padded);
      return;
    }
    this.device.queue.writeBuffer(buffer, offset, data.buffer ?? data, data.byteOffset ?? 0, byteLength);
  }
