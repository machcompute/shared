...
/** Absolute 256-key chunks touched by a prompt/decode dispatch. */
export function gemmaAttentionChunkRange(basePos, rows, window = 0) {
  const newestPosition = basePos + rows - 1;
  const firstPosition = window > 0
    ? Math.max(0, basePos + 1 - window)
    : 0;
  const base = Math.floor(firstPosition / 256);
  const last = Math.floor(newestPosition / 256);
...
export class GemmaModel {
  ...
  #setU(basePos, rows) {
    ...
    const local = gemmaAttentionChunkRange(basePos, rows, this.cfg.text.slidingWindow);
  ...
  #forward(rows, basePos, withHead, encoder = null) {
    ...
    const globalChunks = gemmaAttentionChunkRange(basePos, rows);
    const localChunks = gemmaAttentionChunkRange(basePos, rows, T.slidingWindow);
    for (const e of this.layers) {
      ...
      run(P.attn[kind], e.attn, rows, T.kvHeads, chunks.count);
  ...
  async decodeBatch(firstToken, k, params = {}) {
    ...
    for (let i = 0; i < k; i++) {
      ...
      const local = gemmaAttentionChunkRange(position, 1, this.cfg.text.slidingWindow);
