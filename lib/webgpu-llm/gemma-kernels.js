...
// Shared by the WGSL generator and its command-encoder dispatch contract.
export const GEMM_Q4_TILE_N = 128;
export const GEMM_Q4_TILE_T = 32;
 ...
 * assignment vs +=. `ROW_LANES=64` is the portable four-row layout;
 * fixed-32-lane adapters can use eight rows and `DIRECT_SUBGROUP=1` so each
 * row is reduced and written by one subgroup without a workgroup barrier.
 */
export const gemvQ4 = ({
  N,
  K,
  ...
  if (ROW_LANES !== 32 && ROW_LANES !== 64) throw new Error('gemvQ4 ROW_LANES must be 32 or 64');
  if (DIRECT_SUBGROUP && (!SUBGROUPS || ROW_LANES !== 32)) {
    throw new Error('gemvQ4 DIRECT_SUBGROUP requires SUBGROUPS and ROW_LANES=32');
  }
  if (I8 && GATED) throw new Error('gemvQ4 I8 and GATED cannot be combined');
  const ROWS_PER_WORKGROUP = 256 / ROW_LANES;
  return /* wgsl */`
${I8 ? 'requires packed_4x8_integer_dot_product;' : ''}
...
${I8 ? '@group(0) @binding(4) var<storage, read> xsm: array<u32>;' : ''}
...
fn loadX(base: u32, i: u32) -> vec4f {
  let v = x[base + i];
  return vec4f(
    gemma_gelu_tanh(v.x), gemma_gelu_tanh(v.y),
    gemma_gelu_tanh(v.z), gemma_gelu_tanh(v.w)
  ) * x[base + XSTRIDE + i];
}` : `
fn loadX(base: u32, i: u32) -> vec4f { return x[base + i]; }
`}
...
fn main(@builtin(workgroup_id) wid: vec3u, @builtin(local_invocation_id) lid: vec3u
  ...
  let rowIn = wid.y;
  let rowInBase = rowIn * ${GATED ? 2 : 1}u * XSTRIDE;
  let localRow = lid.x / ${ROW_LANES}u;
  let lane = lid.x % ${ROW_LANES}u;
  let row = wid.x * ${ROWS_PER_WORKGROUP}u + localRow;
  var acc = 0.0;
  if (row < ${N}u) {
    let wb = row * WPR;
    let sb = row * BPR;
...
${I8 ? `
      // rmsnormQ packs each 16-value activation block in the nibble order
      // needed by four DP4a operations. Its f16 pair stores the symmetric
      // activation scale and the integer sum used by affine Q4's min term.
      let b16 = w >> 1u;
      let xqb = rowIn * ${K / 4}u + b16 * 4u;
      let sx = unpack2x16float(xsm[rowIn * ${K / 16}u + b16]);
      let idot = dot4I8Packed(a & 0x0F0F0F0Fu, x[xqb])
               + dot4I8Packed((a >> 4u) & 0x0F0F0F0Fu, x[xqb + 1u])
               + dot4I8Packed(b & 0x0F0F0F0Fu, x[xqb + 2u])
               + dot4I8Packed((b >> 4u) & 0x0F0F0F0Fu, x[xqb + 3u]);
      acc += sm.x * sx.x * f32(idot) + sm.y * sx.x * sx.y;
` : `
      let x0 = loadX(rowInBase, w * 2u);
      let x1 = loadX(rowInBase, w * 2u + 1u);
      let x2 = loadX(rowInBase, w * 2u + 2u);
      let x3 = loadX(rowInBase, w * 2u + 3u);
      let qsum =
...
` : `
  let subgroupCount = ${ROW_LANES}u / sgs;
  if (sgi == 0u) { red[lid.x / sgs] = acc; }
  workgroupBarrier();
  if (lane == 0u && row < ${N}u) {
    var total = 0.0;
    for (var group = 0u; group < subgroupCount; group++) {
      total += red[localRow * subgroupCount + group];
    }
    let dst = rowIn * ${OSTRIDE}u + ${OUTOFF}u + row;
    ${RESIDUAL ? 'out[dst] += total;' : 'out[dst] = total;'}
  }
`}
` : `
  red[lid.x] = acc;
  workgroupBarrier();
  var step = ${ROW_LANES / 2}u;
  loop {
    if (step == 0u) { break; }
...
/**
 * Decode-only fused affine-Q4 gate/up projection and GELU multiplication.
 *
 * Each fixed 32-lane subgroup computes one gate/up pair: its lower and upper
 * 16-lane halves consume complete 32-value Q4 blocks for gate[i] and up[i].
 * Four XOR shuffles reduce both halves independently, then lane zero writes
 * GELU(gate) * up without materializing the 2*I projection. Eight pairs are
 * produced per 256-thread workgroup.
 *
 * Bindings: qdata[2*I,K], scales[2*I,K/32], xq[K/4], out[I], xsm[K/16].
 */
export const gemvQ4GateUpI8 = ({ INTERM, K }) => {
  requireInt('INTERM', INTERM);
  assertQ4Shape(2 * INTERM, K);
  return /* wgsl */`
...
@group(0) @binding(3) var<storage, read_write> out: array<f32>;
@group(0) @binding(4) var<storage, read> xsm: array<u32>;
const WPR = ${K / 8}u;
const BPR = ${K / 32}u;
...
fn main(@builtin(workgroup_id) wid: vec3u, @builtin(local_invocation_id) lid: vec3u,
  ...
  let localPair = lid.x >> 5u;
  let halfLane = sgi & 15u;
  let output = wid.x * 8u + localPair;
  let row = output + select(0u, ${INTERM}u, sgi >= 16u);
  var acc = 0.0;
  if (output < ${INTERM}u) {
    let wb = row * WPR;
    let sb = row * BPR;
    ...
    loop {
      ...
      let xqb0 = b16 * 4u;
      let xqb1 = xqb0 + 4u;
      let sx0 = unpack2x16float(xsm[b16]);
      let sx1 = unpack2x16float(xsm[b16 + 1u]);
      let idot0 = dot4I8Packed(a & 0x0F0F0F0Fu, xq[xqb0])
                + dot4I8Packed((a >> 4u) & 0x0F0F0F0Fu, xq[xqb0 + 1u])
                + dot4I8Packed(b & 0x0F0F0F0Fu, xq[xqb0 + 2u])
                + dot4I8Packed((b >> 4u) & 0x0F0F0F0Fu, xq[xqb0 + 3u]);
      let idot1 = dot4I8Packed(c & 0x0F0F0F0Fu, xq[xqb1])
                + dot4I8Packed((c >> 4u) & 0x0F0F0F0Fu, xq[xqb1 + 1u])
                + dot4I8Packed(d & 0x0F0F0F0Fu, xq[xqb1 + 2u])
                + dot4I8Packed((d >> 4u) & 0x0F0F0F0Fu, xq[xqb1 + 3u]);
      acc += sm.x * (sx0.x * f32(idot0) + sx1.x * f32(idot1))
           + sm.y * (sx0.x * sx0.y + sx1.x * sx1.y);
      w += 64u;
    }
  }
  ...
  let gate = subgroupShuffle(acc, 0u);
  let up = subgroupShuffle(acc, 16u);
  if (sgi == 0u && output < ${INTERM}u) {
    out[output] = gemma_gelu_tanh(gate) * up;
  }
}`;
};

/**
 * Tiled affine-Q4 GEMM for prompt prefill.
 *
 * A workgroup covers 32 prompt rows by 128 output rows. Each thread owns a
 * 4x4 register tile while the workgroup cooperatively dequantizes a 128x16
 * weight tile and loads a 32x16 activation tile. Reusing each weight value
 * across 32 prompt rows is the material difference from the old reference
 * kernel, which re-read the same packed weights once for every four rows.
 *
 * Bindings: qdata, scales, x[T,K], out[T,OSTRIDE], u where u.rows is T.
 * Dispatch `(ceil(N / 128), ceil(T / 32))`.
 */
...
// Padding the K stride avoids shared-memory bank conflicts during the inner
// product. Total workgroup storage is 10,880 bytes.
var<workgroup> xTile: array<f32, ${GEMM_Q4_TILE_T * 17}>;
var<workgroup> wTile: array<f32, ${GEMM_Q4_TILE_N * 17}>;
const WPR = ${K / 8}u;
const BPR = ${K / 32}u;

@compute @workgroup_size(256)
fn main(@builtin(workgroup_id) wid: vec3u, @builtin(local_invocation_id) lid: vec3u) {
  let n0 = wid.x * ${GEMM_Q4_TILE_N}u;
  let t0 = wid.y * ${GEMM_Q4_TILE_T}u;
  let nx = lid.x & 31u;
  let ty = lid.x >> 5u;
  ...
  for (var k0 = 0u; k0 < ${K}u; k0 += 16u) {
    // Load X[32,16]: two scalar values per thread.
    for (var e = 0u; e < 2u; e++) {
      let index = lid.x * 2u + e;
      let tileRow = index >> 4u;
      let tileK = index & 15u;
      let row = t0 + tileRow;
      xTile[tileRow * 17u + tileK] = select(
        0.0,
        x[row * ${K}u + k0 + tileK],
        row < u.rows
      );
    ...
    // Load and dequantize W[128,16]. Each thread expands one packed u32;
    // adjacent threads cover the two eight-value halves of a row.
    let tileRow = lid.x >> 1u;
    let half = lid.x & 1u;
    let row = n0 + tileRow;
    if (row < ${N}u) {
      let sm = unpack2x16float(scales[row * BPR + (k0 >> 5u)]);
      let packed = qdata[row * WPR + (k0 >> 3u) + half];
      for (var j = 0u; j < 8u; j++) {
        let q = f32((packed >> (4u * j)) & 15u);
        wTile[tileRow * 17u + half * 8u + j] = sm.x * q + sm.y;
      }
    } else {
      for (var j = 0u; j < 8u; j++) {
        wTile[tileRow * 17u + half * 8u + j] = 0.0;
      }
    }
    workgroupBarrier();

    // Four output rows by four prompt rows per thread.
    for (var kk = 0u; kk < 16u; kk++) {
      let x0 = xTile[ty * 17u + kk];
      let x1 = xTile[(ty + 8u) * 17u + kk];
      let x2 = xTile[(ty + 16u) * 17u + kk];
      let x3 = xTile[(ty + 24u) * 17u + kk];
      for (var i = 0u; i < 4u; i++) {
        let w = wTile[(nx + i * 32u) * 17u + kk];
        acc[i * 4u] += w * x0;
        acc[i * 4u + 1u] += w * x1;
        acc[i * 4u + 2u] += w * x2;
        acc[i * 4u + 3u] += w * x3;
  ...
  for (var i = 0u; i < 4u; i++) {
    let outputRow = n0 + nx + i * 32u;
    if (outputRow >= ${N}u) { continue; }
    for (var j = 0u; j < 4u; j++) {
      let promptRow = t0 + ty + j * 8u;
      if (promptRow < u.rows) {
        let dst = promptRow * ${OSTRIDE}u + ${OUTOFF}u + outputRow;
        ${RESIDUAL ? 'out[dst] += acc[i * 4u + j];' : 'out[dst] = acc[i * 4u + j];'}
      }
    }
  }
}`;
...
/** Decode fusion: post-attention RMSNorm+residual and pre-FFN int8 RMSNorm. */
export const rmsnormAddQ = ({ K, EPS = 1e-6 }) => {
  requireInt('K', K);
  if (K % 16) throw new Error(`rmsnormAddQ K (${K}) must be divisible by 16`);
  return /* wgsl */`
struct U { rows: u32, pad0: u32, pad1: u32, pad2: u32 }
@group(0) @binding(0) var<storage, read> x: array<vec4f>;
@group(0) @binding(1) var<storage, read> postGamma: array<f32>;
@group(0) @binding(2) var<storage, read> residual: array<f32>;
@group(0) @binding(3) var<storage, read_write> out: array<f32>;
@group(0) @binding(4) var<storage, read> preGamma: array<f32>;
@group(0) @binding(5) var<storage, read_write> xq: array<u32>;
@group(0) @binding(6) var<storage, read_write> xsm: array<u32>;
@group(0) @binding(7) var<uniform> u: U;
var<workgroup> red: array<f32, 256>;
var<workgroup> ysh: array<f32, ${K}>;
...
fn main(@builtin(workgroup_id) wid: vec3u, @builtin(local_invocation_id) lid: vec3u) {
  let row = wid.x;
  if (row >= u.rows) { return; }
  let vecBase = row * ${K / 4}u;
  let base = row * ${K}u;
  var ss = 0.0;
  for (var i = lid.x; i < ${K / 4}u; i += 256u) {
    ss += dot(x[vecBase + i], x[vecBase + i]);
  }
  red[lid.x] = ss;
  workgroupBarrier();
  var step = 128u;
  ...
  let postRi = inverseSqrt(red[0] / ${K}.0 + ${Number(EPS)});
  for (var i = lid.x; i < ${K}u; i += 256u) {
    let y = residual[base + i]
      + x[vecBase + (i >> 2u)][i & 3u] * postRi * postGamma[i];
    ysh[i] = y;
    out[base + i] = y;
  }
  workgroupBarrier();
  ss = 0.0;
  for (var i = lid.x; i < ${K}u; i += 256u) { ss += ysh[i] * ysh[i]; }
  red[lid.x] = ss;
  workgroupBarrier();
  step = 128u;
  loop { if (step == 0u) { break; }
  ...
  let preRi = inverseSqrt(red[0] / ${K}.0 + ${Number(EPS)});
  let xqRow = row * ${K / 4}u;
  let xsmRow = row * ${K / 16}u;
  for (var b = lid.x; b < ${K / 16}u; b += 256u) {
    var y: array<f32, 16>;
    var mx = 0.0;
    for (var i = 0u; i < 16u; i++) {
      let d = b * 16u + i;
      y[i] = ysh[d] * preRi * preGamma[d];
      mx = max(mx, abs(y[i]));
    }
    let sc = mx / 127.0;
