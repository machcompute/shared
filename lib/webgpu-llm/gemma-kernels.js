 ...
 */
export const gemvQ4 = ({ N, K, RESIDUAL = 0, OSTRIDE = N, OUTOFF = 0, SUBGROUPS = 0, GATED = 0 }) => {
  assertQ4Shape(N, K);
...
var<workgroup> xtile: array<vec4f, 32>;
const WPR = ${K / 8}u;
const BPR = ${K / 32}u;
const XSTRIDE = ${K / 4}u;
...
fn main(@builtin(workgroup_id) wid: vec3u, @builtin(local_invocation_id) lid: vec3u) {
  ...
  let valid = n < ${N}u && t < u.rows;
  var acc = 0.0;
  let flat = lid.y * 64u + lid.x;
  for (var block = 0u; block < BPR; block++) {
    // Thirty-two vec4 values cover four input rows × one 32-value Q4 block.
    // Loading them cooperatively avoids re-reading the same activation block
    // for every one of this workgroup's 64 output neurons.
    if (flat < 32u) {
      let tileRow = flat >> 3u;
      let tileVec = flat & 7u;
      let sourceRow = wid.y * 4u + tileRow;
      if (sourceRow < u.rows) {
        xtile[flat] = x[sourceRow * XSTRIDE + block * 8u + tileVec];
      } else {
        xtile[flat] = vec4f(0.0);
      }
    }
    workgroupBarrier();

    if (valid) {
      let qb = n * WPR + block * 4u;
      let a = qdata[qb];
      let b = qdata[qb + 1u];
      let c = qdata[qb + 2u];
      let d = qdata[qb + 3u];
      let sm = unpack2x16float(scales[n * BPR + block]);
      let tb = lid.y * 8u;
      let x0 = xtile[tb];
      let x1 = xtile[tb + 1u];
      let x2 = xtile[tb + 2u];
      ...
      let qsum =
        ...
        + dot(vec4f(f32(b & 15u), f32((b >> 4u) & 15u), f32((b >> 8u) & 15u), f32((b >> 12u) & 15u)), x2)
        + dot(vec4f(f32((b >> 16u) & 15u), f32((b >> 20u) & 15u), f32((b >> 24u) & 15u), f32((b >> 28u) & 15u)), x3)
        + dot(vec4f(f32(c & 15u), f32((c >> 4u) & 15u), f32((c >> 8u) & 15u), f32((c >> 12u) & 15u)), x4)
        + dot(vec4f(f32((c >> 16u) & 15u), f32((c >> 20u) & 15u), f32((c >> 24u) & 15u), f32((c >> 28u) & 15u)), x5)
        + dot(vec4f(f32(d & 15u), f32((d >> 4u) & 15u), f32((d >> 8u) & 15u), f32((d >> 12u) & 15u)), x6)
        + dot(vec4f(f32((d >> 16u) & 15u), f32((d >> 20u) & 15u), f32((d >> 24u) & 15u), f32((d >> 28u) & 15u)), x7);
      let xsum = dot(x0 + x1 + x2 + x3 + x4 + x5 + x6 + x7, vec4f(1.0));
      acc += sm.x * qsum + sm.y * xsum;
    }
    workgroupBarrier();
  }
 ...
 * partials[T][HEADS][MAXCTX/256][HEAD_DIM+2], u(basePos,T).
 * Dispatch `(T, KV_HEADS, floor((basePos + T - 1) / 256) + 1)`.
 */
