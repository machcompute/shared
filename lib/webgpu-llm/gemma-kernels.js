...
/**
 * Flash-decoding style causal GQA attention over the FP8 E4M3FN cache, pass 1.
 *
 ...
 * reduction per key.  This kernel instead splits the context into 256-key
 ...
 * (`position / 256`), so sliding windows simply skip leading chunks.
 *
 * Bindings: q[T,QDIM], kCache, vCache,
 * partials[T][HEADS][MAXCTX/256][HEAD_DIM+2],
 * u(basePos,T,chunkBase). Dispatch `(T, KV_HEADS, activeChunks)`; `wid.z`
 * is offset by `chunkBase` so sliding layers need not launch old chunks.
 */
export const textFlashAttention = ({
  ...
  requireInt('WINDOW', WINDOW, 0);
  if (WINDOW > MAXCTX) throw new Error(`WINDOW (${WINDOW}) cannot exceed MAXCTX (${MAXCTX})`);
  const MAXCHUNKS = Math.ceil(MAXCTX / 256);
  const PSTRIDE = HEAD_DIM + 2;
  const comps = ['x', 'y', 'z', 'w'];
  // E4B KV groups hold 4 Q heads (one vec4 batch); E2B holds 8 (two batches).
  const batches = [];
  for (let first = 0; first < GROUPS; first += 4) {
    batches.push({ first, count: Math.min(4, GROUPS - first) });
  }
  const QSH = Math.min(GROUPS, 4) * WORDS;
  ...
  { // Q heads ${first}..${first + count - 1} of this KV group.
    ...
    if (valid) {
      for (var w = 0u; w < ${WORDS}u; w++) {
        ...
        let kv = vec4f(
          gemma_fp8e4m3fn_decode_lane(kw, 0u), gemma_fp8e4m3fn_decode_lane(kw, 1u),
          gemma_fp8e4m3fn_decode_lane(kw, 2u), gemma_fp8e4m3fn_decode_lane(kw, 3u));
        s.x += dot(qsh[w], kv);
        ${count > 1 ? `s.y += dot(qsh[${WORDS}u + w], kv);` : ''}
        ${count > 2 ? `s.z += dot(qsh[${2 * WORDS}u + w], kv);` : ''}
        ${count > 3 ? `s.w += dot(qsh[${3 * WORDS}u + w], kv);` : ''}
      }
      s *= kScale * ${Number(ATTENTION_SCALE)};
    }
    probs[lid.x] = select(vec4f(-1.0e30), s, valid);
    workgroupBarrier();
    ...
    let m = red[0];
    let pr = select(vec4f(0.0), exp(s - m), valid);
    probs[lid.x] = pr;
    workgroupBarrier();
    ...
    // below reads only packed FP8 words.
    probs[lid.x] = pr * vScale;
    workgroupBarrier();
    ${heads.map((qh) => `let rec${qh} = ((t * ${HEADS}u + kvh * ${GROUPS}u + ${qh}u) * ${MAXCHUNKS}u + chunk) * ${PSTRIDE}u;`).join('\n    ')}
    var acc0 = vec4f(0.0);
    ${HEAD_DIM > 256 ? 'var acc1 = vec4f(0.0);' : ''}
    if (lid.x < ${HEAD_DIM}u) {
      for (var i = lo; i <= hi; i++) {
        let vb = ((chunkBase + i) * ${KV_HEADS}u + kvh) * WORDS;
        let pv = probs[i];
        acc0 += pv * gemma_fp8e4m3fn_decode_lane(vCache[vb + (lid.x >> 2u)], lid.x & 3u);
        ${HEAD_DIM > 256 ? `acc1 += pv * gemma_fp8e4m3fn_decode_lane(vCache[vb + ${256 / 4}u + (lid.x >> 2u)], lid.x & 3u);` : ''}
      }
      ${heads.map((qh, h) => `partials[rec${qh} + lid.x] = acc0.${comps[h]};`).join('\n      ')}
      ${HEAD_DIM > 256 ? heads.map((qh, h) => `partials[rec${qh} + lid.x + 256u] = acc1.${comps[h]};`).join('\n      ') : ''}
    }
    if (lid.x == 0u) {
      ${heads.map((qh, h) => `partials[rec${qh} + ${HEAD_DIM}u] = m.${comps[h]};
      partials[rec${qh} + ${HEAD_DIM + 1}u] = dsum.${comps[h]};`).join('\n      ')}
    }
    workgroupBarrier();
  }`;
  };
...
var<workgroup> probs: array<vec4f, 256>;
var<workgroup> red: array<vec4f, 128>;
const WORDS = ${WORDS}u;
const CACHE_WORDS = ${CACHE_WORDS}u;
...
fn main(@builtin(workgroup_id) wid: vec3u, @builtin(local_invocation_id) lid: vec3u) {
  ...
  let start = ${WINDOW === 0
    ? '0u'
    : `select(0u, pos + 1u - ${WINDOW}u, pos + 1u > ${WINDOW}u)`};
  if (chunk < (start >> 8u) || chunk > (pos >> 8u)) { return; }
  let chunkBase = chunk * 256u;
  let p = chunkBase + lid.x;
  let valid = p >= start && p <= pos;
  ...
  let hi = min(pos, chunkBase + 255u) - chunkBase;
  let qWordBase = t * ${QDIM / 4}u + kvh * ${GROUPS * WORDS}u;
${batches.map(batchBlock).join('\n')}
}`;
};
 ...
 */
export const textFlashCombine = ({ HEADS = 8, HEAD_DIM, MAXCTX, WINDOW = 0 }) => {
  requireInt('HEADS', HEADS);
  requireInt('HEAD_DIM', HEAD_DIM);
  requireInt('MAXCTX', MAXCTX);
  requireInt('WINDOW', WINDOW, 0);
