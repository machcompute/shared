...
export const topk = ({ VOCAB, KTOP, ALLOWED = false }) => /*wgsl*/`
@group(0) @binding(0) var<storage, read> logits: array<f32>;
@group(0) @binding(1) var<storage, read_write> cand: array<u32>; // [row][blocks][KTOP][2]
${ALLOWED ? `@group(0) @binding(2) var<storage, read> allowed: array<u32>;
@group(0) @binding(3) var<uniform> allowedCount: u32;
fn isAllowed(id: u32) -> bool {
  var lo = 0u;
  var hi = allowedCount;
  loop {
    if (lo >= hi) { break; }
    let mid = lo + (hi - lo) / 2u;
    let value = allowed[mid];
...
fn main(@builtin(workgroup_id) wid: vec3u, @builtin(local_invocation_id) lid: vec3u) {
  ...
  for (var e = 0u; e < 4u; e++) {
    let i = lid.x * 4u + e;
    let g = base + i;
    vals[i] = select(-3.0e38, logits[rowOff + g], g < ${VOCAB}u${ALLOWED ? ' && isAllowed(g)' : ''});
  }
  workgroupBarrier();
