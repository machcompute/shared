...
import { gemmGGML } from './ggml/kernels.js';

const F = Float32Array.BYTES_PER_ELEMENT;
const ceilDiv = (n, d) => Math.ceil(n / d);
const u32 = (...values) => new Uint32Array(values);

function matrixPart(weight, firstRow, rows) {
  const parts = weight?.segments ?? [weight];
  const part = parts.find((entry) => (entry.outOffset ?? 0) === firstRow && entry.N === rows);
  if (!part?.buffer || part.shards) throw new Error(`Gemma media projection is missing native rows ${firstRow}..${firstRow + rows}.`);
  return part;
}
...
/**
 * Reference WebGPU media encoders. Each projection retains its GGML type and every
...
export class GemmaModalities {
  ...
  #pipelines() {
    ...
    this.p = {
      gemm: (weight, N, K, options = {}) => named(
        'ggml.gemm',
        { type: weight.type, N, K, ...options },
        () => gemmGGML({ N, K, TYPE: weight.type, ...options }),
      ),
      ...
      visionPos: named('vision.position', {}, () => KS.addVisionPositions({ H: 768, TABLE_SIZE: 10240 })),
  ...
  #matmul(pass, rows, N, K, weight, input, output, uRows, firstRow = 0) {
    const matrix = matrixPart(weight, firstRow, N);
    const pipeline = this.p.gemm(matrix, N, K);
    this.#run(
      pass,
      pipeline,
      this.#bind(pipeline, [matrix.buffer, input, output, uRows]),
      ceilDiv(N, 128),
      ceilDiv(rows, 32),
    );
  }
  ...
  async encodeImage(input) {
    ...
    try {
      ...
      this.#run(pass, this.p.visionPos, this.#bind(this.p.visionPos, [b.x, b.positions, this.w['vision.positionEmb'].buffer, b.xAlt, b.uRows]), ceilDiv(V.hidden, 256), rows);
